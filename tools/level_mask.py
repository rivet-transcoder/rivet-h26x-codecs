"""Do two Annex-B streams differ only in the level they claim?
...
- H.265 video parameter set: `general_tier_flag` (bit 0x20 of RBSP byte 6)
  and `general_level_idc` (RBSP byte 17), counting the two NAL header bytes.
  The profile_tier_level starts at bit 32 of the VPS (7.3.2.1).
- H.265 sequence parameter set: the same two fields at RBSP bytes 3 and 14;
  its profile_tier_level starts at bit 8 (7.3.2.2).
...
# (name, byte index of the tier flag or None, byte index of the level) per
# NAL unit type, in the unescaped unit including its header.
FIELDS = {
    "h264": {7: ("SPS", None, 3)},
    "h265": {32: ("VPS", 6, 17), 33: ("SPS", 3, 14)},
}
...
def main():
    ...
    for k, (x, y) in enumerate(zip(a, b)):
        ...
        changes.add(f"{name} level {ux[level]}->{uy[level]}{tiers}")
