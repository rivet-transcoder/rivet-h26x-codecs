...
| | |
|---|---|
| `verify.sh [--baseline FILE] [--no-suites] [decoder]` | Everything that has to hold before a change lands: every fixture against its recorded MD5 (`golden.txt` beside the script — the committed one — unless `GOLD` names another), the SIMD rung sweep over the fixtures, then all four conformance suites. Prints one tally per suite and `ALL GREEN` or not. `--no-suites` stops after the rungs. Safe to run concurrently with another copy of itself. |
...
| `make_fixtures.sh <dir>` | Generates the fixture set: x264 / x265 encodes of deterministic lavfi sources, one per coding tool, with every encode checked against the wrapper's log, the encoder's SEI option string, ffprobe and `stream_census.py` so a fixture provably carries what it is named for. Writes `fixtures.md5` and, on a rerun, reports whether every fixture regenerated byte for byte. |
| `check.sh <file...>` | One or more fixtures against libavcodec's per-frame MD5s (`<name>.framemd5`, generated on first use), reporting the frame counts and the first mismatching frame; exits 1 on any difference. `DEC` and `FFMPEG` name the binaries. |
| `record_golden.sh <decoder> [dir]` | Prints the `golden.txt` lines for every fixture in `dir/fixtures.md5`, hashed the way `verify.sh` compares them. Redirect to `tools/golden.txt` — after `check.sh` is clean, never to make a red run green. |
| `stream_census.py <file>` | What an Annex-B stream carries, read from its parameter sets, NAL headers and slice headers: profile, chroma, depth, CABAC / 8x8 / scaling lists / MBAFF / cropping for H.264; CTU size, WPP, SAO, AMP, TMVP, sign hiding, cu_qp_delta depth, transquant bypass, IDR / CRA / RASL counts, temporal ids for HEVC; pictures, slices per picture and slice types for both. |
...
The fixtures are not vendored either, and the golden list is. The first
set (47 encodes of clips and encoder builds that no longer exist) was lost
with the scratch directory on 2026-09-09 and could not be recreated, so
`make_fixtures.sh` now makes the set from lavfi sources with fixed
settings — 53 streams, 1.7 MB — and `tools/golden.txt` holds what the
verified decoder decodes each to. Regenerate the set with the script, run
`check.sh` on all of it against libavcodec, and only then
`record_golden.sh`. One fixture, `x265_cu_lossless.265`, is where
libavcodec is wrong rather than us (it applies SAO to the chroma of
transquant-bypass CUs in inter pictures; our samples there equal the
encoder's source frame, which is what lossless means) — the generator
writes that, with the evidence, to `fixtures.libav_deviates`, and
`check.sh` reports it as `DEVIATES` instead of failing.
